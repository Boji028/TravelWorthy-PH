...
class EmailVerificationToken(db.Model):
    ...
    def generate_token(user_id: int, email: str, expires_in_hours: int = 24) -> "EmailVerificationToken":
        ...
        verification_token = EmailVerificationToken(
            user_id=user_id, token=token, email=email, expires_at=expires_at, is_used=False
        )
    ...
    def get_valid_token(token: str) -> Optional["EmailVerificationToken"]:
        ...
        verification = EmailVerificationToken.query.filter_by(token=token, is_used=False).first()
