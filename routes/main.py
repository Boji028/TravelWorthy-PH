...
def home():
    ...
    featured = TourPackage.query.options(selectinload(TourPackage.images)).filter_by(is_active=True, is_featured=True).order_by(TourPackage.created_at.desc()).limit(6).all()
    packages = featured if featured else TourPackage.query.options(selectinload(TourPackage.images)).filter_by(is_active=True).order_by(TourPackage.created_at.desc()).limit(6).all()
    ...
    return render_template('main/home.html', packages=packages, testimonials=testimonials, posts=posts,
                           continents=continents, site_settings=site_settings)
        ...
        for orphan in image_filenames:
            try:
                ...
                current_app.logger.warning(
                    f"Could not clean up orphaned upload {orphan}: {cleanup_err}"
                )
...
def track_inquiry(reference_number):
    ...
    viewing_as_admin = (
        ...
        and inquiry.user_id != current_user.id
    )
...
def mark_notifications_read():
    ...
    InquiryNotification.query.filter_by(
...
def mark_notification_read(notification_id):
    ...
    from models.inquiry_notification import InquiryNotification
    notif = InquiryNotification.query.filter_by(
