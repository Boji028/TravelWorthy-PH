    ...
    if form.validate_on_submit():
        try:
            ...
            current_app.logger.info(
                f"New inquiry created: id={inquiry.id}, from={inquiry.email}, " f"destination={inquiry.destination}"
            )
...
def inquire_package(package_id):
    ...
    if form.validate_on_submit():
        try:
            ...
            db.session.add(inquiry)
            db.session.commit()
            current_app.logger.info(f"New inquiry for package {package_id}: id={inquiry.id}, from={inquiry.email}")
