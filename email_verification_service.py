...
class EmailVerificationService:
    ...
    def create_verification_token(user_id: int, email: str, expires_in_hours: int = 24) -> str:
        ...
        try:
            ...
            EmailVerificationToken.query.filter_by(user_id=user_id, is_used=False).update({"is_used": True})
            db.session.commit()
            ...
            StructuredLogger.log_auth_event(
                "verification_token_created", email, True, reason=f"Verification token created for user {user_id}"
            )
    ...
    def send_verification_email(
        user_email: str, user_name: str, token: str, is_resend: bool = False, expires_in_hours: int = 24
    ) -> bool:
...
<body style="font-family: Arial, sans-serif; line-height: 1.6;">
    ...
    <p>Best regards,<br>Travel Agency Team</p>
            ...
            StructuredLogger.log_error(
                "email_verification", f"Failed to send verification email: {str(e)}", {"email": user_email}, "ERROR"
            )
            return False
    ...
    def verify_email(token: str) -> Tuple[bool, str, Optional[User]]:
        ...
        try:
            ...
            StructuredLogger.log_auth_event("email_verified", user.email, True, reason="Email successfully verified")
    ...
    def resend_verification_email(email: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Create new token
            token = EmailVerificationService.create_verification_token(user.id, user.email, expires_in_hours=24)

            # Send email
            if not EmailVerificationService.send_verification_email(user.email, user.name, token, is_resend=True):
            ...
            StructuredLogger.log_error(
                "email_verification", f"Failed to resend verification email: {str(e)}", {"email": email}, "ERROR"
            )
    ...
    def cleanup_expired_tokens() -> int:
        ...
        try:
            ...
            result = EmailVerificationToken.query.filter(EmailVerificationToken.expires_at < now).delete()

            db.session.commit()

            StructuredLogger.log_admin_action("cleanup", "email_verification_tokens", None, {"tokens_deleted": result})

            return result

        except Exception as e:
            StructuredLogger.log_error("email_verification", f"Failed to cleanup expired tokens: {str(e)}", {}, "ERROR")
            return 0
