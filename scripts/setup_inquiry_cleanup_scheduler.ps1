# PowerShell script to create Windows Task Scheduler entry for inquiry cleanup
# Run this as Administrator

param(
    [string]$Time = "03:00",  # Default: 3:00 AM (an hour after the 2:00 AM backup)
    [switch]$Help
)
...
if ($Help) {
    Write-Host @"
USAGE:
    powershell -ExecutionPolicy Bypass -File setup_inquiry_cleanup_scheduler.ps1

PARAMETERS:
    -Time "HH:MM"  : Set run time (default: 03:00)
...
EXAMPLES:
    ...
    # Run at 4:30 AM
    powershell -ExecutionPolicy Bypass -File setup_inquiry_cleanup_scheduler.ps1 -Time "04:30"
"@
    exit 0
}
...
if (-not $isAdmin) {
    ...
    Write-Host "3. Run: powershell -ExecutionPolicy Bypass -File setup_inquiry_cleanup_scheduler.ps1"
    exit 1
}

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "🗑️  Travel Agency Inquiry Cleanup Setup" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# Configuration
$ProjectPath = "C:\Users\ENZO KOPS\Desktop\TryNewWebsite\travel_agency_enhanced\fixed"
$BatchFile = "$ProjectPath\scripts\run_inquiry_cleanup.bat"
$TaskName = "Travel Agency Inquiry Cleanup"
$TaskDescription = "Daily: warn admins about, then delete, inquiries older than 3 months"
