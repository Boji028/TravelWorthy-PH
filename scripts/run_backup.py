...
def main():
    ...
    try:
        ...
        if success:
            ...
            StructuredLogger.log_admin_action("backup_executed", "database", None, {"message": message})
            return 0
        else:
            print(f"Backup failed: {message}")
            StructuredLogger.log_error("backup", f"Backup execution failed: {message}", {}, "ERROR")
            return 1

    except Exception as e:
        print(f"Error running backup: {e}")
        StructuredLogger.log_error("backup", f"Error running backup: {str(e)}", {}, "ERROR")
        return 1
