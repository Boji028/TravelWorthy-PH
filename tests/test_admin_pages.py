...
class TestTestimonialAdminDelete:
    def test_admin_can_delete_via_ajax(self, app, admin_client, test_user):
        ...
        response = admin_client.post(f"/testimonial/delete/{testimonial_id}", headers={"X-Requested-With": "XMLHttpRequest"})
        assert response.status_code == 200
    ...
    def test_non_admin_cannot_delete(self, app, authenticated_client, admin_user):
        ...
        response = authenticated_client.post(
            f"/testimonial/delete/{testimonial_id}", headers={"X-Requested-With": "XMLHttpRequest"}
        )
