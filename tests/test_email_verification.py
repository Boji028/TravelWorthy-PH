...
class TestEmailVerificationToken:
    ...
    def test_token_expiration(self, test_user, app):
        ...
        token_obj = EmailVerificationToken.generate_token(test_user.id, test_user.email, expires_in_hours=1)
    ...
    def test_get_valid_token(self, test_user, app):
        ...
        token_obj = EmailVerificationToken.generate_token(test_user.id, test_user.email)
        from app import db
...
class TestEmailVerificationService:
    ...
    def test_create_verification_token(self, test_user, app):
        ...
        token = EmailVerificationService.create_verification_token(test_user.id, test_user.email)
    ...
    def test_create_verification_token_invalid_user(self, app):
        ...
        with pytest.raises(ValueError):
            EmailVerificationService.create_verification_token(9999, "invalid@example.com")
    ...
    def test_resend_verification_email_success(self, test_user, app, monkeypatch):
        ...
        monkeypatch.setattr(EmailVerificationService, "send_verification_email", staticmethod(lambda *args, **kwargs: True))
        ...
        success, message = EmailVerificationService.resend_verification_email(test_user.email)
    ...
    def test_cleanup_expired_tokens(self, test_user, app):
        ...
        expired_token = EmailVerificationToken.generate_token(
            test_user.id, test_user.email, expires_in_hours=-1  # Already expired
        )
        db.session.add(expired_token)
        db.session.commit()

        # Create a valid token
        valid_token = EmailVerificationToken.generate_token(test_user.id, test_user.email, expires_in_hours=24)
...
class TestEmailVerificationRoutes:
    ...
    def test_resend_verification_route(self, client, test_user, monkeypatch):
        ...
        response = client.post("/auth/resend-verification", data={"email": test_user.email}, follow_redirects=True)
    ...
    def test_resend_verification_invalid_email(self, client):
        ...
        response = client.post("/auth/resend-verification", data={"email": "nonexistent@example.com"}, follow_redirects=True)
