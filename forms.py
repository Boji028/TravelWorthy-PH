...
class RegisterForm(FlaskForm):
    ...
    phone = StringField(
        "Phone", validators=[Optional(), Regexp(r"^\+?\d{7,15}$", message="Invalid phone number format (e.g. +639171234567)")]
    )
    password = PasswordField("Password", validators=[DataRequired("Password is required"), StrongPasswordValidator()])
...
class LoginForm(FlaskForm):
    ...
    password = PasswordField(
        "Password",
        validators=[DataRequired("Password is required"), Length(max=128, message="Password must be 128 characters or fewer")],
    )
...
class ChangePasswordForm(FlaskForm):
    ...
    new_password = PasswordField(
        "New Password", validators=[DataRequired("New password is required"), StrongPasswordValidator()]
    )
    confirm_password = PasswordField(
        "Confirm New Password",
        validators=[
...
class InquiryForm(FlaskForm):
    ...
    travel_date_from = DateField("Travel From", validators=[DataRequired("Start date is required")])
    travel_date_to = DateField("Travel To", validators=[DataRequired("End date is required")])
    num_adults = IntegerField(
        "Number of Adults",
        validators=[DataRequired("Number of adults is required"), NumberRange(min=1, max=100, message="Must be 1-100 adults")],
    )
    num_children = IntegerField(
        "Number of Children", default=0, validators=[Optional(), NumberRange(min=0, max=100, message="Must be 0-100 children")]
    )
    num_infants = IntegerField(
        "Number of Infants", default=0, validators=[Optional(), NumberRange(min=0, max=100, message="Must be 0-100 infants")]
    )
...
class TourPackageForm(FlaskForm):
    ...
    title = StringField(
        "Package Title",
        validators=[DataRequired("Title is required"), Length(min=5, max=200, message="Title must be 5-200 characters")],
    )
    ...
    duration_days = IntegerField(
        "Duration (days)",
        validators=[DataRequired("Duration is required"), NumberRange(min=1, max=365, message="Duration must be 1-365 days")],
    )
    price = FloatField(
        "Price", validators=[DataRequired("Price is required"), NumberRange(min=0.01, message="Price must be greater than 0")]
    )
    currency = SelectField("Currency", choices=[("PHP", "PHP"), ("USD", "USD"), ("EUR", "EUR")], default="PHP")
    inclusions = TextAreaField(
        "Inclusions", validators=[Optional(), Length(max=2000, message="Inclusions must be under 2000 characters")]
    )
    exclusions = TextAreaField(
        "Exclusions", validators=[Optional(), Length(max=2000, message="Exclusions must be under 2000 characters")]
    )
    image = FileField(
        "Package Image",
        validators=[FileAllowed(["jpg", "jpeg", "png", "gif", "webp"], "Image files only (jpg, jpeg, png, gif, webp)")],
    )
