...
class BackupService:
    ...
    def _parse_db_url(db_url: str) -> Dict:
        ...
        try:
            ...
            StructuredLogger.log_error(
                "backup", f"Failed to parse database URL: {str(e)}", {"db_url": db_url[:20] + "..."}, "ERROR"
            )
            return {}
    ...
    def create_backup(self, compress: bool = True, include_metadata: bool = True) -> Tuple[bool, str, Optional[Path]]:
        ...
        try:
            ...
            StructuredLogger.log_admin_action(
                ...
                "database",
                None,
                {"backup_name": backup_name, "file_size_mb": round(file_size_mb, 2), "compressed": compress},
            )
    ...
    def restore_backup(self, backup_file: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            StructuredLogger.log_admin_action("backup_restored", "database", None, {"backup_file": str(backup_path)})
        ...
        try:
            ...
            for metadata_file in sorted(self.backup_dir.glob("backup_*.json"), reverse=True):
                try:
                    ...
                    backup_files = [f for f in self.backup_dir.glob(f"{backup_name}.*") if f.suffix != ".json"]
                    ...
                    StructuredLogger.log_error(
                        "backup", f"Failed to read backup metadata: {str(e)}", {"file": str(metadata_file)}, "WARNING"
                    )
            ...
            StructuredLogger.log_error("backup", f"Failed to list backups: {str(e)}", {}, "ERROR")
            return []
        ...
        try:
            ...
            for backup_file in self.backup_dir.glob("backup_*"):
                ...
                try:
                    ...
                    StructuredLogger.log_error(
                        "backup", f"Failed to delete old backup: {str(e)}", {"file": str(backup_file)}, "WARNING"
                    )
            ...
            StructuredLogger.log_admin_action(
                ...
                "database",
                None,
                {"deleted_count": deleted_count, "freed_space_mb": round(freed_space_mb, 2), "retention_days": retention_days},
            )

            return deleted_count, round(freed_space_mb, 2)

        except Exception as e:
            StructuredLogger.log_error("backup", f"Failed to cleanup old backups: {str(e)}", {}, "ERROR")
            return 0, 0
    ...
    def verify_backup(self, backup_file: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            StructuredLogger.log_admin_action("backup_verified", "database", None, {"backup_file": str(backup_path)})

            return True, f"Backup file is valid ({file_size / (1024 * 1024):.2f} MB)"

        except Exception as e:
            StructuredLogger.log_error("backup", f"Failed to verify backup: {str(e)}", {"backup_file": backup_file}, "ERROR")
    ...
    def get_backup_stats(self) -> Dict:
        ...
        try:
            ...
            StructuredLogger.log_error("backup", f"Failed to get backup stats: {str(e)}", {}, "ERROR")
            return {}
